...
class DPOTrainer(Trainer):
    ...
    def __init__(
        ...
        alpha: float = 0.5, # alpha for alpha-divergence loss
        ...
        # alpha for alpha-divergence loss
        self.alpha = alpha
        self.debug = debug
    ...
    def dpo_loss(
        ...
        if self.loss_type == "sigmoid":
            ...
            #  logsigmoid [ beta * ( f'(-1/u) - f'(-1/u) ) ]
            # where u := pi_logratios, ref_logratios
            # calculate loss
        ...
        elif self.loss_type == "alpha-divergence":
            pi_logratios = torch.exp(pi_logratios)
            ref_logratios = torch.exp(ref_logratios)
            
            losses = -F.logsigmoid(self.beta * (((torch.reciprocal(ref_logratios + epsilon) - torch.reciprocal(pi_logratios + epsilon)) * self.alpha)))

            # reward beta * ((1 - 1/u^a) * a)
            chosen_rewards = self.beta * ((1 - torch.reciprocal(torch.pow(policy_chosen_logps / (reference_chosen_logps + epsilon), self.alpha)) * self.alpha)).detach()
            rejected_rewards = self.beta * ((1 - torch.reciprocal(torch.pow(policy_rejected_logps / (reference_rejected_logps + epsilon), self.alpha)) * self.alpha)).detach()
